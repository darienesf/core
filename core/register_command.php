...
if (\OC::$server->getConfig()->getSystemValue('installed', false)) {
	...
	$application->add(new OC\Core\Command\User\Add(\OC::$server->getGroupManager(), \OC::$server->getMailer(),
		new \OC\User\Service\CreateUserService(
			\OC::$server->getUserSession(), \OC::$server->getGroupManager(),
			\OC::$server->getUserManager(), \OC::$server->getSecureRandom(),
			\OC::$server->getLogger(),
			new \OC\User\Service\UserSendMailService(
			...
			new \OC\User\Service\PasswordGeneratorService(\OC::$server->getEventDispatcher(),
